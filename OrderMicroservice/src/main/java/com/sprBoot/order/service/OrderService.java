...
import com.sprBoot.order.kafka.UserEventConsumer;
...
public class OrderService {
    ...
    public OrderService(OrderRepository orderRepository,
			CircuitBreakerFactory<?, ?> circuitBreakerFactory, UserEventConsumer userEventConsumer) {
		super();
    ...
    public Order createOrder(Order order) {
        if (!userEventConsumer.isValidUser(order.getUserId())) {
	 ...
	 * 
	 * // Verify user existence log.info("Calling User Service for ID: {}",
	 * order.getUserId()); ResponseEntity<Object> response =
	 * restTemplate.getForEntity( "http://localhost:8081/api/users/" +
	 * order.getUserId(), Object.class );
	 * 
	 * if(!response.getStatusCode().is2xxSuccessful()) {
	 * log.error("User validation failed for ID: {}", order.getUserId()); throw new
	 * ResourceNotFoundException("User not found with ID: " + order.getUserId()); }
	 * 
